 ...
 *
 *    1. builtin    — the runtime itself: `node:fs`, and bare `crypto` / `path`
 *    2. external   — npm packages: `vue`, `express`, `@vue/test-utils`
 *    3. workspace  — this monorepo's other packages: `@ereuna/shared`
 *    4. local      — this package: `@/…`, `./…`, `../…`, and `#…` — the subpath
 *                    imports packages/shared declares in its package.json
 *
...
export const IMPORT_GROUPS = ['builtin', 'external', 'workspace', 'local'];

const WORKSPACE_SCOPE = '@ereuna/';
...
function groupOf(spec: string): number {
    ...
    if (spec.startsWith(WORKSPACE_SCOPE)) return 2;
    if (spec.startsWith('@/') || spec.startsWith('.') || spec.startsWith('#')) return 3;
    return 1;
}
