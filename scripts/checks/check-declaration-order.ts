 ...
 *     ../lib/declaration-order.ts.
 *
 * INSIDE LAYER 1. Imports are the one category whose internal order is
 * mechanical, so a third pass sorts them: builtin → external → workspace → local,
 ...
 * pass is the only one `--fix` touches — `node scripts/checks/check-declaration-order.ts --fix`
...
import { CATEGORIES, TEST_CATEGORIES, analyzeFile } from '../lib/declaration-order.ts';
...
const isTest = (rel: string): boolean => rel.includes('__tests__') || /\.(test|spec)\.ts$/.test(rel);
