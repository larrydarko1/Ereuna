 ...
 */
const TEST_ONLY: Category & { key: string } = {
    key: 'testOnlyExports',
...
/** One knip finding, flattened. `name === file` means the whole file is unused. */
...
function knip(include: string[], extra: string[] = []): Record<string, Hit[]> {
    ...
    const found: Record<string, Hit[]> = Object.fromEntries(include.map((key) => [key, []]));
...
if (failed) {
    console.error(
        'Budgets live in scripts/checks/check-dead-code.ts and only ever go down.\n' +
