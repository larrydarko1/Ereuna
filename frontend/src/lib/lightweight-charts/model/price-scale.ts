...
export class PriceScale {
    ...
    public constructor(
        ...
        localizationOptions: LocalizationOptionsBase,
    ) {
        this._id = id;
    ...
    public applyOptions(options: DeepPartial<PriceScaleOptions>): void {
        ...
        if (options.scaleMargins !== undefined) {
            const top = getDefined(options.scaleMargins.top);
            const bottom = getDefined(options.scaleMargins.bottom);
    ...
    public setMode(newMode: Partial<PriceScaleState>): void {
        ...
        let priceRange: PriceRangeImpl | null;
        ...
        if (newMode.mode !== undefined) {
            ...
            // Can go once rebuildTickMarks is lazy
    ...
    public internalHeight(): number {
        if (this._internalHeightCache !== null) {
    ...
    public setPriceRange(newPriceRange: PriceRangeImpl | null): void {
        const oldPriceRange = this._priceRange;

        // Nothing to do when the range has not moved — except the very first
        // time it is set, which still has to drop the marks
        if (
        ...
        this.forcePriceRange(newPriceRange);
    }

    /**
     * Sets the range whether or not it changed.
     *
     * Dragging the scale re-applies a shifted copy of the same snapshot every
     * frame, and one of those has to land even when the arithmetic puts it back
     * on the range that is already in place.
     */
    public forcePriceRange(newPriceRange: PriceRangeImpl | null): void {
        this._marksCache = null;
    ...
    public isEmpty(): boolean {
        this._makeSureItIsValid();
        return this._height === 0 || this._priceRange === null || this._priceRange.isEmpty();
    }
    ...
    public pointsArrayToCoordinates<T extends PricedValue>(
        ...
        const range = getNotNull(this.priceRange());
    ...
    public startScale(x: number): void {
        ...
        this._priceRangeSnapshot = getNotNull(this.priceRange()).clone();
    }
    ...
    public scaleTo(x: number): void {
        ...
        const newPriceRange = getNotNull(this._priceRangeSnapshot).clone();
    ...
    public scrollTo(x: number): void {
        ...
        const priceUnitsPerPixel = getNotNull(this.priceRange()).length() / (this.internalHeight() - 1);
    ...
    public formatPriceAbsolute(price: number): string {
        return this._formatPrice(price as BarPrice, getNotNull(this._formatterSource()).formatter());
    }
    ...
    private _logicalToCoordinate(logical: number, _baseValue: number): Coordinate {
        ...
        logical = this.isLog() && logical !== 0 ? toLog(logical, this._logFormula) : logical;
        const range = getNotNull(this.priceRange());
        const invCoordinate =
            this._bottomMarginPx() + ((this.internalHeight() - 1) * (logical - range.minValue())) / range.length();
    ...
    private _coordinateToLogical(coordinate: number, _baseValue: number): number {
        ...
        const logical =
            range.minValue() +
            range.length() * ((invCoordinate - this._bottomMarginPx()) / (this.internalHeight() - 1));
    ...
    private _recalculatePriceRangeImpl(): void {
        ...
        for (const source of sources) {
            ...
            let sourceRange = autoScaleInfo?.priceRange() ?? null;
            ...
            if (sourceRange !== null) {
                ...
                if (priceRange === null) {
                    ...
                    priceRange = priceRange.merge(getNotNull(sourceRange));
                }
        ...
        if (priceRange !== null) {
            ...
            if (priceRange.minValue() === priceRange.maxValue()) {
                const formatterSource = this._formatterSource();
                const minMove =
                    formatterSource === null || this.isPercentage() || this.isIndexedTo100()
                        ? 1
            ...
            if (this.isLog()) {
                priceRange = this._relogPriceRange(priceRange);
            }
    ...
    /**
     * Re-derives the log formula from the range now on screen and re-expresses
     * that range — and any drag snapshot taken under the old formula — in it.
     *
     * Returns the range unchanged when the formula did not move.
     */
    private _relogPriceRange(priceRange: PriceRangeImpl | null): PriceRangeImpl | null {
        ...
        const rawSnapshot =
            this._priceRangeSnapshot !== null
                ? convertPriceRangeFromLog(this._priceRangeSnapshot, this._logFormula)
                : null;
        ...
        return convertPriceRangeToLog(rawRange, newLogFormula);
    }
    ...
    private _getCoordinateTransformer(): PriceTransformer | null {
        ...
        if (this.isLog()) {
            return (price: number): number => toLog(price, this._logFormula);
        }
    ...
    private _formatValue(
        value: BarPrice | number,
        formatter: PriceFormatterFn | undefined,
