...
const { hasDrawings = false, hasSignals = false, patternsShown = false, replaying = false } = defineProps<{
    hasDrawings?: boolean;
    hasSignals?: boolean;
...
<template>
    <div class="toolbar" role="toolbar" :aria-label="t('charts.tools.label')">
        <div class="toolbar__group">
            <button
                ...
                @click="pick(option)"
            >
                <svg class="toolbar__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                    <template v-if="option === 'ruler'">
                        <path d="M3 15.5 15.5 3 21 8.5 8.5 21z" />
                        <path d="M7 11.5 9 13.5M10.5 8 12.5 10M14 4.5 16 6.5" />
                    </template>
                    <path v-else-if="option === 'trendline'" d="M4 20 20 4" />
                    <rect v-else-if="option === 'box'" x="3" y="3" width="18" height="18" rx="1" />
                    <path v-else-if="option === 'text'" d="M4 7V4h16v3M9 20h6M12 4v16" />
