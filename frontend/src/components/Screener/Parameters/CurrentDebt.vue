<template>
  <div :class="[showCurrentDebtModel ? 'param-card-expanded' : 'param-card']">
    <div class="header">
      ...
      <label class="switch">
        <input type="checkbox" id="current-debt-check" v-model="showCurrentDebtModel" aria-label="Toggle Current Debt filter">
        <span class="slider"></span>
      </label>
    </div>
    
    <div class="content" v-if="showCurrentDebtModel">
      <div class="input-group">
        <div class="input-wrapper">
          <label class="input-label">Minimum</label>
          <input class="input-field" id="left-cd" type="number" step="0.01" placeholder="0.00" aria-label="Current Debt minimum">
        </div>
        <div class="input-wrapper">
          <label class="input-label">Maximum</label>
          <input class="input-field" id="right-cd" type="number" step="0.01" placeholder="0.00" aria-label="Current Debt maximum">
        </div>
      </div>
      
      <div class="actions">
        <button class="btn btn-secondary" @click="emit('reset'); emit('update:showCurrentDebt', false)" aria-label="Reset Current Debt filter">
          Reset
        </button>
        <button class="btn btn-primary" @click="SetCurrentDebt()" aria-label="Set Current Debt filter">
          Apply
        </button>
      </div>
    </div>
