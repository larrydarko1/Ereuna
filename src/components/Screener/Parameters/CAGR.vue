<template>
   <div :class="[showCAGRModel ? 'param-s1-expanded' : 'param-s1']">
          <div class="row">
            <div
              ...
              <svg class="question-img" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                aria-label="CAGR Info" @mouseover="handleMouseOver($event, 'cagr')" @mouseout="handleMouseOut">
            ...
            <label style="float:right" class="switch">
              <input type="checkbox" id="cagr-check" v-model="showCAGRModel" style="border: none;" aria-label="Toggle CAGR">
              <span class="slider round"></span>
            </label>
          </div>
          <div style="border: none;" v-if="showCAGRModel">
            <div class="row">
              <input class="left input" id="left-cagr" type="text" placeholder="min %" aria-label="CAGR Min">
              <input class="right input" id="right-cagr" type="text" placeholder="max %" aria-label="CAGR Max">
            </div>
            <div class="row">
              <button class="btns" style="float:right" @click="SetCAGR()" aria-label="Set CAGR">
              ...
              <button class="btnsr" style="float:right" @click="emit('reset'); emit('update:showCAGR', false)" aria-label="Reset CAGR">
...
const showCAGRModel = computed({
  get: () => props.showCAGR,
  set: (val: boolean) => emit('update:showCAGR', val)
});
...
// adds and modifies CAGR value for screener
async function SetCAGR() {
  error.value = '';
  ...
  const leftCAGR = leftValue === '' ? null : parseFloat(leftValue) / 100;
  const rightCAGR = rightValue === '' ? null : parseFloat(rightValue) / 100;
  ...
  try {
    const response = await fetch('/api/screener/cagr', {
      method: 'PATCH',
      headers: {
