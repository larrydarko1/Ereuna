<template>
  <div :class="[showNetExpenseRatioInputsModel ? 'param-s1-expanded' : 'param-s1']">
          <div class="row">
            <div
              style="float:left; font-weight: bold; position:absolute; top: 0px; left: 5px; display: flex; flex-direction: row; align-items: center;">
              <p>Net Expense Ratio (%)</p>
              <svg class="question-img" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                @mouseover="handleMouseOver($event, 'netExpenseRatio')" @mouseout="handleMouseOut"
                aria-label="Info: Net Expense Ratio parameter">
            ...
            <label style="float:right" class="switch">
              <input type="checkbox" id="net-expense-ratio-check" v-model="showNetExpenseRatioInputsModel" style="border: none;">
              <span class="slider round"></span>
            </label>
          </div>
          <div style="border: none;" v-if="showNetExpenseRatioInputsModel">
            <div class="row">
              <input class="left input" id="left-net-expense-ratio" type="text" placeholder="min" aria-label="Minimum Net Expense Ratio">
              <input class="right input" id="right-net-expense-ratio" type="text" placeholder="max" aria-label="Maximum Net Expense Ratio">
            </div>
            <div class="row">
              <button class="btns" style="float:right" @click="SetNetExpenseRatio()" aria-label="Set Net Expense Ratio range">
              ...
              <button class="btnsr" style="float:right" @click="emit('reset'); emit('update:showNetExpenseRatioInputs', false)" aria-label="Reset Net Expense Ratio filter">
...
const props = defineProps<{
  ...
  showNetExpenseRatioInputs: boolean;
}>();
...
// adds and modifies net expense ratio value for screener 
async function SetNetExpenseRatio() {
  ...
  try {
    const response = await fetch('/api/screener/net-expense-ratio', {
      method: 'PATCH',
      headers: {
...
const showNetExpenseRatioInputsModel = computed({
  get: () => props.showNetExpenseRatioInputs,
  set: (val: boolean) => emit('update:showNetExpenseRatioInputs', val)
});
