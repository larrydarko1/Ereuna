...
<template>
  <div :class="[showROAModel ? 'param-s1-expanded' : 'param-s1']">
    <div class="row">
      <div style="float:left; font-weight: bold; position:absolute; top: 0px; left: 5px; display: flex; flex-direction: row; align-items: center;">
        ...
        <svg class="question-img" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
          @mouseover="handleMouseOver($event, 'roa')" @mouseout="handleMouseOut($event)" aria-label="Show info for ROA parameter">
      ...
      <label style="float:right" class="switch">
        <input type="checkbox" id="price-check" :checked="showROAModel" @change="emit('update:showROA', ($event.target as HTMLInputElement)?.checked)" style="border: none;" aria-label="Toggle ROA filter">
        <span class="slider round"></span>
      </label>
    </div>
    <div style="border: none;" v-if="showROAModel">
      <div class="row">
        <input class="left input" id="left-roa" type="text" placeholder="min" aria-label="ROA minimum">
        <input class="right input" id="right-roa" type="text" placeholder="max" aria-label="ROA maximum">
      </div>
      <div class="row" style="flex-direction: row;">
        <button class="btns" style="float:right" @click="SetROA()" aria-label="Set ROA filter">
        ...
        <button class="btnsr" style="float:right" @click="emit('reset'); emit('update:showROA', false)" aria-label="Reset ROA filter">
...
const emit = defineEmits(['fetchScreeners', 'handleMouseOver', 'handleMouseOut', 'reset', 'notify', 'update:showROA']);
...
// adds and modifies ROA value for screener 
async function SetROA() {
  ...
  const leftROA = leftValue === '' ? null : parseFloat(leftValue);
  const rightROA = rightValue === '' ? null : parseFloat(rightValue);
  // If both missing or both invalid, error
  if ((leftROA === null && rightROA === null) ||
      (leftROA !== null && isNaN(leftROA) && rightROA !== null && isNaN(rightROA))) {
  ...
  if (leftROA !== null && !isNaN(leftROA) && rightROA !== null && !isNaN(rightROA)) {
    if (leftROA >= rightROA) {
      error.value = 'Min ROA cannot be higher than or equal to max ROA';
      showNotification(error.value);
      emit('fetchScreeners', props.selectedScreener);
      return;
    }
...
const showROAModel = computed({
  get: () => props.showROA,
  set: (val: boolean) => emit('update:showROA', val)
});
