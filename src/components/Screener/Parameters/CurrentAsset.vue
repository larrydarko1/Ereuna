<template>
  <div :class="[showCurrentAssetsModel ? 'param-card-expanded' : 'param-card']">
    <div class="header">
      <div class="title-section">
        <span class="title">{{ t('params.currentAssets') }}</span>
        <svg class="info-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
          @mouseover="handleMouseOver($event, 'current-assets')" @mouseout="handleMouseOut" aria-label="Show info for Current Assets parameter">
          <path
      ...
      <label class="switch">
        <input type="checkbox" id="current-assets-check" v-model="showCurrentAssetsModel" aria-label="Toggle Current Assets filter">
        <span class="slider"></span>
      </label>
    </div>
    
    <div class="content" v-if="showCurrentAssetsModel">
      <div class="input-group">
        <div class="input-wrapper">
          <label class="input-label">Minimum</label>
          <input class="input-field" id="left-ca" type="number" step="0.01" placeholder="0.00" aria-label="Current Assets minimum">
        </div>
        <div class="input-wrapper">
          <label class="input-label">Maximum</label>
          <input class="input-field" id="right-ca" type="number" step="0.01" placeholder="0.00" aria-label="Current Assets maximum">
        </div>
      </div>
      
      <div class="actions">
        <button class="btn btn-secondary" @click="emit('reset'); emit('update:showCurrentAssets', false)" aria-label="Reset Current Assets filter">
          Reset
        </button>
        <button class="btn btn-primary" @click="SetCurrentAssets()" aria-label="Set Current Assets filter">
          Apply
        </button>
      </div>
    </div>
...
async function SetCurrentAssets() {
  ...
  const rightInput = document.getElementById('right-ca') as HTMLInputElement | null;
  ...
  if (leftAssets !== null && !isNaN(leftAssets) && rightAssets !== null && !isNaN(rightAssets)) {
    if (leftAssets >= rightAssets) {
      error.value = t('params.errorMinMaxCurrentAssets');
      showNotification(error.value);
      emit('fetchScreeners', props.selectedScreener);
      return;
    }
