...
async function SetCurrentRatio() {
  ...
  const leftCurrentRatio = leftValue === '' ? null : parseFloat(leftValue);
  const rightCurrentRatio = rightValue === '' ? null : parseFloat(rightValue);
...
const showCurrentRatioModel = computed({
  get: () => props.showCurrentRatio,
  set: (val: boolean) => emit('update:showCurrentRatio', val)
});
