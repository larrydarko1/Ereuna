<template>
  <div :class="[showPEInputsModel ? 'param-card-expanded' : 'param-card']">
    <div class="header">
      ...
      <label class="switch">
        <input type="checkbox" id="pe-check" v-model="showPEInputsModel" aria-label="Toggle PE Ratio filter">
        <span class="slider"></span>
      </label>
    </div>

    <div class="content" v-if="showPEInputsModel">
      <div class="input-group">
        <div class="input-wrapper">
          <label class="input-label">Minimum</label>
          <input class="input-field" id="left-pe" type="number" step="0.01" placeholder="0.00" aria-label="PE Ratio minimum">
        </div>
        <div class="input-wrapper">
          <label class="input-label">Maximum</label>
          <input class="input-field" id="right-pe" type="number" step="0.01" placeholder="0.00" aria-label="PE Ratio maximum">
        </div>
      </div>

      <div class="actions">
        <button class="btn btn-secondary" @click="emit('reset'); emit('update:showPEInputs', false)" aria-label="Reset PE Ratio filter">
          Reset
        </button>
        <button class="btn btn-primary" @click="SetPE()" aria-label="Set PE Ratio filter">
          Apply
        </button>
      </div>
    </div>
