<template>
  <div :class="[showProfitMarginModel ? 'param-card-expanded' : 'param-card']">
    <div class="header">
      ...
      <label class="switch">
        <input type="checkbox" id="profit-margin-check" v-model="showProfitMarginModel" aria-label="Toggle Profit Margin filter">
        <span class="slider"></span>
      </label>
    </div>
    
    <div class="content" v-if="showProfitMarginModel">
      <div class="input-group">
        <div class="input-wrapper">
          <label class="input-label">Minimum</label>
          <input class="input-field" id="left-pm" type="number" step="0.01" placeholder="0.00" aria-label="Profit Margin minimum">
        </div>
        <div class="input-wrapper">
          <label class="input-label">Maximum</label>
          <input class="input-field" id="right-pm" type="number" step="0.01" placeholder="0.00" aria-label="Profit Margin maximum">
        </div>
      </div>
      
      <div class="actions">
        <button class="btn btn-secondary" @click="emit('reset'); emit('update:showProfitMargin', false)" aria-label="Reset Profit Margin filter">
          Reset
        </button>
        <button class="btn btn-primary" @click="SetProfitMargin()" aria-label="Set Profit Margin filter">
          Apply
        </button>
      </div>
    </div>
