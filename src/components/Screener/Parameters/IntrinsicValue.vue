<template>
  <div :class="[showIntrinsicValueModel ? 'param-card-expanded' : 'param-card']">
    ...
    <div class="content" v-if="showIntrinsicValueModel">
      <div class="input-group">
        <div class="input-wrapper">
          <label class="input-label">Minimum</label>
          <input class="input-field" id="left-iv" type="number" step="0.01" placeholder="0.00" aria-label="Intrinsic Value minimum">
        </div>
        <div class="input-wrapper">
          <label class="input-label">Maximum</label>
          <input class="input-field" id="right-iv" type="number" step="0.01" placeholder="0.00" aria-label="Intrinsic Value maximum">
        </div>
      </div>
      
      <div class="actions">
        <button class="btn btn-secondary" @click="emit('reset'); emit('update:showIntrinsicValue', false)" aria-label="Reset Intrinsic Value filter">
          Reset
        </button>
        <button class="btn btn-primary" @click="SetIntrinsicValue()" aria-label="Set Intrinsic Value filter">
          Apply
        </button>
      </div>
    </div>
