          ...
          <div class="row">
            <div
              ...
              <svg class="question-img" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                @mouseover="handleMouseOver($event, 'iv')" @mouseout="handleMouseOut">
            ...
            <label style="float:right" class="switch">
              <input type="checkbox" v-model="showIntrinsicValue">
              <span class="slider round"></span>
            </label>
          </div>
          <div style="border: none;" v-if="showIntrinsicValue">
            <div class="row">
              <input class="left input" id="left-iv" type="text" placeholder="min">
              <input class="right input" id="right-iv" type="text" placeholder="max">
            </div>
            <div class="row">
              <button class="btns" style="float:right" @click="SetIntrinsicValue()">
              ...
              <button class="btnsr" style="float:right" @click="emit('reset'), showIntrinsicValue = false">
...
async function SetIntrinsicValue() {
  try {
    ...
    const leftIntrinsicValue = parseFloat(document.getElementById('left-iv').value)
    const rightIntrinsicValue = parseFloat(document.getElementById('right-iv').value)

    if (leftIntrinsicValue >= rightIntrinsicValue) {
      throw new Error('Min cannot be higher than or equal to max')
    }

    const response = await fetch('/api/screener/intrinsic-value', {
      method: 'PATCH',
      headers: {
      ...
      body: JSON.stringify({
        minPrice: leftIntrinsicValue,
        maxPrice: rightIntrinsicValue,
    ...
    if (data.message === 'updated successfully') {
      ...
      throw new Error('Error updating Intrinsic Value range')
    }
