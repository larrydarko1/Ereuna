<template>
 <div :class="[ShowFundFamily ? 'param-s1-expanded' : 'param-s1']">
          <div class="row">
            <div
        ...
        <svg class="question-img" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
          @mouseover="handleMouseOver($event, 'fundFamily')" @mouseout="handleMouseOut" aria-label="Show info for Fund Family parameter">
            ...
            <label style="float:right" class="switch">
              <input type="checkbox" v-model="ShowFundFamily" aria-label="Toggle Fund Family filter">
              <span class="slider round"></span>
            </label>
          </div>
          <div style="border: none" v-if="ShowFundFamily">
            <div class="row2">
              <div class="check" v-for="(family, index) in FundFamilies" :key="index">
                <div :id="`fund-family-${index}`" class="custom-checkbox" :class="{ checked: selectedFundFamilies[index] }"
                  @click="toggleFundFamily(index)" :aria-label="`Toggle ${family} fund family`" role="checkbox" :aria-checked="selectedFundFamilies[index]">
            ...
            <div class="row">
              <button class="btns" style="float:right" @click="SetFundFamily" aria-label="Set Fund Family filter">
              ...
              <button class="btnsr" style="float:right" @click="emit('reset'), ShowFundFamily = false" aria-label="Reset Fund Family filter">
...
const props = defineProps({
  ...
  ShowFundFamily: { type: Boolean, required: true },
  initialSelected: { type: Array as () => string[], required: false, default: () => [] }
})

const ShowFundFamily = computed({
  get: () => props.ShowFundFamily,
  set: (val: boolean) => emit('update:ShowFundFamily', val)
});
const FundFamilies = ref<string[]>([]); // hosts all available fund families 
const selectedFundFamilies = ref<boolean[]>([]);
const error = ref('');
...
// Load hardcoded fund families data (instant, no API call)
function loadFundFamilies() {
  FundFamilies.value = getFundFamilies();
  selectedFundFamilies.value = new Array(FundFamilies.value.length).fill(false);
  if (props.initialSelected && props.initialSelected.length > 0) {
    selectedFundFamilies.value = FundFamilies.value.map(f => props.initialSelected.includes(f));
  }
}
...
watch(() => props.initialSelected, (val: string[] | undefined) => {
  if (!val || !Array.isArray(val) || FundFamilies.value.length === 0) return;
  selectedFundFamilies.value = FundFamilies.value.map(f => val.includes(f));
});

const toggleFundFamily = (index: number) => {
  selectedFundFamilies.value[index] = !selectedFundFamilies.value[index];
};

// sends fund families data to update screener
async function SetFundFamily() {
  const selected = FundFamilies.value.filter((_, index) => selectedFundFamilies.value[index]);
  error.value = '';
  ...
  try {
    const response = await fetch('/api/screener/fund-family', {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'X-API-KEY': props.apiKey,
      },
      body: JSON.stringify({ fundFamilies: selected, screenerName: props.selectedScreener, user: props.user })
    });
