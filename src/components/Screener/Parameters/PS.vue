<template>
  <div :class="[showPSInputsModel ? 'param-card-expanded' : 'param-card']">
    <div class="header">
      ...
      <label class="switch">
        <input type="checkbox" id="ps-check" v-model="showPSInputsModel" aria-label="Toggle PS Ratio filter">
        <span class="slider"></span>
      </label>
    </div>

    <div class="content" v-if="showPSInputsModel">
      <div class="input-group">
        <div class="input-wrapper">
          <label class="input-label">Minimum</label>
          <input class="input-field" id="left-ps" type="number" step="0.01" placeholder="0.00" aria-label="PS Ratio minimum">
        </div>
        <div class="input-wrapper">
          <label class="input-label">Maximum</label>
          <input class="input-field" id="right-ps" type="number" step="0.01" placeholder="0.00" aria-label="PS Ratio maximum">
        </div>
      </div>

      <div class="actions">
        <button class="btn btn-secondary" @click="emit('reset'); emit('update:showPSInputs', false)" aria-label="Reset PS Ratio filter">
          Reset
        </button>
        <button class="btn btn-primary" @click="SetPSRatio()" aria-label="Set PS Ratio filter">
          Apply
        </button>
      </div>
    </div>
