<template>
  <div :class="[showPBInputsModel ? 'param-card-expanded' : 'param-card']">
    <div class="header">
      ...
      <label class="switch">
        <input type="checkbox" id="pb-check" v-model="showPBInputsModel" aria-label="Toggle PB Ratio filter">
        <span class="slider"></span>
      </label>
    </div>
    
    <div class="content" v-if="showPBInputsModel">
      <div class="input-group">
        <div class="input-wrapper">
          <label class="input-label">Minimum</label>
          <input class="input-field" id="left-pb" type="number" step="0.01" placeholder="0.00" aria-label="PB Ratio minimum">
        </div>
        <div class="input-wrapper">
          <label class="input-label">Maximum</label>
          <input class="input-field" id="right-pb" type="number" step="0.01" placeholder="0.00" aria-label="PB Ratio maximum">
        </div>
      </div>
      
      <div class="actions">
        <button class="btn btn-secondary" @click="emit('reset'); emit('update:showPBInputs', false)" aria-label="Reset PB Ratio filter">
          Reset
        </button>
        <button class="btn btn-primary" @click="SetPBRatio()" aria-label="Set PB Ratio filter">
          Apply
        </button>
      </div>
    </div>
