<template>
  <div :class="[ShowSectorModel ? 'param-card-expanded' : 'param-card']">
    <div class="header">
      <div class="title-section">
        <span class="title">{{ t('params.sector') }}</span>
        <svg class="info-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
          @mouseover="handleMouseOver($event, 'sector')" @mouseout="handleMouseOut($event)" aria-label="Show info for Sector parameter">
          <path
      ...
      <label class="switch">
        <input type="checkbox" v-model="ShowSectorModel" aria-label="Toggle Sector filter">
        <span class="slider"></span>
      </label>
    </div>

    <div class="content" v-if="ShowSectorModel">
      <div class="checkbox-group">
        <div class="checkbox-item" v-for="(sector, index) in Sectors" :key="index">
          <label class="checkbox-label" :for="`sector-${index}`">
            <input
              :id="`sector-${index}`"
              type="checkbox"
              v-model="selectedSectors[index]"
              class="checkbox-input"
              :aria-label="'Select ' + sector + ' sector'"
            >
      ...
      <div class="actions">
        <button class="btn btn-secondary" @click="emit('reset'); ShowSectorModel = false" aria-label="Reset Sector filter">
          {{ t('params.reset') }}
        </button>
        <button class="btn btn-primary" @click="SetSector()" aria-label="Set Sector filter">
          {{ t('params.apply') }}
        </button>
      </div>
    </div>
...
const ShowSectorModel = computed({
